...
class E_envoice_cr_mapper {
	...
	protected function generateFechaEmision(&$data)
	{
		...
		$dtime = DateTime::createFromFormat($date_format . ' ' . $time_format, $data['transaction_time']);
		$sale_timestamp = $dtime->getTimestamp();
		$date = format_document_date($sale_timestamp);
		return $date;
	}
